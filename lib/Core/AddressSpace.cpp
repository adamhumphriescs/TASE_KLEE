...
void AddressSpace::bindObject(const MemoryObject *mo, ObjectState *os) {
  //TASE: We're using 1 process per execution state, so we don't
  //need COW support on memory objects.
...
ObjectState *AddressSpace::getWriteable(const MemoryObject *mo,
  ...
  //TASE: Vanilla KLEE code is left commented out for comparison.
  //We're using 1 process per execution state in TASE so we don't
  //use the software based COW functionality in KLEE.
